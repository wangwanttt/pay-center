...
public class WxPayManager {
    ...
    public static Map<String, String> createNativeOrder(WxPayConfig wxPayConfig, String orderNo, int amount,
                                                        String ip,Project prjVo) throws Exception {
        // 生成微信「统一下单」请求数据
        ...
        dataMap.put("attach",prjVo.getId()+","+prjVo.getCallFunc());
    ...
    public static Map<String, String> createJsAPIOrder(WxPayConfig wxPayConfig, String orderNo, int amount,
                                                       String ip, String openId,Project prjVo) throws Exception {
        // 生成微信「统一下单」请求数据
        Map<String, String> dataMap = new HashMap<>(16);
    ...
    public static Map<String, String> getPayResult(WxPayConfig wxPayConfig, String orderNo)
        ...
        // 返回结果 8726A265AC24D48A00D46C52B6AC102B
     ...
     * 得到alipay 配置
     **/
    public static WxPayConfig getInfo(WxPayConfig config, Project prjVo) {
       config.setAppId(prjVo.getAppid());
        config.setMchId(prjVo.getMchId());
        config.setBody(prjVo.getSubject());
        config.setKey(prjVo.getPubKey());
        config.setNotifyUrl(prjVo.getCallFunc());

        if (prjVo.getPayType() == 21) {  // 微信 NATIVE 支付(二维码支付)
            config.setTradeTypeNative("NATIVE");

        }
        if (prjVo.getPayType() == 22) {  //  微信 JSAPI 支付   (公众号)
            config.setTradeTypeJsApi("JSAPI");
        }
        if (prjVo.getPayType() == 23) {  // 微信 H5 支付
            config.setTradeTypeH5("MWEB");
        }
        if (prjVo.getPayType() == 24) {  // 微信 APP 支付
            config.setTradeTypeApp("APP");
        }
        if (prjVo.getPayType() == 25) {  //  微信 小程序 支付
            config.setTradeTypeJsApi("JSAPI");
        }
